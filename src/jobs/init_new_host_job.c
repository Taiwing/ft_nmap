...
/*   Updated: 2021/11/15 08:26:23 by yforeau          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_nmap.h"

static void	set_host_job_data(t_host_job *host_job, const char *host, t_nmap_config *cfg)
{
...
t_list	*init_new_host_job(t_scan_job *scan)
{
	...
	if ((new = ft_lst_pop(&scan->cfg->empty_host_jobs, 0)))
		host_job = (t_host_job *)new->content;
	if (!(host = next_host(&host_job->host_ip, scan->cfg)))
	{
		ft_memdel((void **)&new);
		return (NULL);
	}
	set_host_job_data(host_job, host, scan->cfg);
	if (!new)
	{
		host_job->port_jobs = (t_port_job *)
			ft_memalloc(sizeof(t_port_job) * scan->cfg->nports);
